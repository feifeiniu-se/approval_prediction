...
def CNN(summary_train, summary_test, descrip_train, descrip_test, y_train, y_test, sentiment_train, sentiment_test, creator_train, creator_test, role_train, role_test, freq_train, freq_test):
    ...
    # 构建word2vec词典
    # W2V_MODEL_s = Word2Vec(summary_train, sg=0, size=EMBEDDING_DIM, window=7, min_count=0, negative=5, sample=0.00025,
    #                        hs=1)
    # W2V_MODEL_d = Word2Vec(descrip_train, sg=0, size=EMBEDDING_DIM, window=7, min_count=0, negative=5, sample=0.00025,
    #                        hs=1)
    W2V_MODEL = Word2Vec(text_train, sg=1, vector_size=EMBEDDING_DIM, window=7, min_count=0, negative=5, sample=0.00025, hs=1)
    ...
    for word, i in word_index_s.items():
        try:
            embedding_vector_s = W2V_MODEL.wv[word]
        except KeyError:
            continue
    ...
    for word, i in word_index_d.items():
        try:
            embedding_vector_d = W2V_MODEL.wv[word]
        except KeyError:
            continue
    ...
    # concatenation = concatenate([conv1D_s, conv1D_d, creator_inputs, role_inputs, freq_inputs])#加creator profile 3 28440
    # concatenation = concatenate([conv1D_s, conv1D_d, conv1D_senti])#加情感分类 2  12864
    ...
    adam = Adam(lr=0.0001)
    model = Model(inputs=[inputs_s, inputs_d, senti_input, creator_inputs, role_inputs, freq_inputs], outputs=predictions)
    ...
    history = model.fit([summary_train, description_train, sentiment_train, creator_train, role_train, freq_train], y_train, batch_size=batch_size, epochs=epoch, verbose=1, callbacks=[early_stopping, reduceLROnPlateau], validation_data=([summary_test, description_test, sentiment_test, creator_test, role_test, freq_test], y_test))
    score = model.evaluate([summary_test, description_test, sentiment_test, creator_test, role_test, freq_test], y_test, verbose=1)
    y_predict = model.predict([summary_test, description_test, sentiment_test, creator_test, role_test, freq_test])
